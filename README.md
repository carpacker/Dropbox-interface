...
- **Files** tab (Workspace): lists directories via Rust commands (`default_local_root`, `list_directory`, `parent_directory`). Each file row shows size + relative-modified time. The **Sort** dropdown (Name / Modified / Size, asc/desc) is shared with every other listing surface; the choice persists to `localStorage` under `dropbox-interface:sort-preference-v1`.
