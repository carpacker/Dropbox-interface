...
- **Job Tracker** (data app, local-only): Pick a folder containing a `jobs.csv`. Rows render as a **board** grouped by a `status` column (auto-detected, case-insensitive; falls back to a single "Backlog" bucket when absent). Empty status cells land in a synthetic Backlog column appended at the end. Click a card → detail panel with all fields, per-job attachments at `<root>/files/<rowKey>/`, and a read-only thread reader at `<root>/threads/<rowKey>.jsonl` (one JSON entry per line: `{ at, by, kind: "note" | "email-link", body }`). Edit a row via the modal (status column renders as a dropdown so changes always land in a real column); CSV rewrites are atomic. Recent Job Trackers surface on the dashboard. Config under `dropbox-interface:job-tracker:v1`; recents under `dropbox-interface:job-tracker:recents:v1`. **Deferred to v2**: add/delete rows, thread writes, drag-between-columns, CRM linkage via `client_id`.
