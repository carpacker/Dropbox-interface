...
- **Browser view toggle.** The local **Files** tab and the **Dropbox** flat browser both support a List / Tile toggle. Tile mode renders square cards with large folder icons / `w256h256` Dropbox thumbnails / extension chips on plain files. The preference is per-browser, persisted under `dropbox-interface:browser-view-mode:v1`.
...
- **Job Tracker** (data app, local-only): Pick a folder containing a `jobs.csv`. Rows render as a **board** grouped by a `status` column (auto-detected, case-insensitive; falls back to a single "Backlog" bucket when absent). Empty status cells land in a synthetic Backlog column appended at the end. Click a card → detail panel with all fields, per-job attachments at `<root>/files/<rowKey>/`, and a read-only thread reader at `<root>/threads/<rowKey>.jsonl` (one JSON entry per line: `{ at, by, kind: "note" | "email-link", body }`). **Writes**: add (toolbar `Add job`, status pre-fills to the first real bucket), edit (detail panel; status renders as a dropdown), delete (detail panel, behind a confirm modal — see [`THREAT_MODEL.md`](THREAT_MODEL.md) §D-L3). Drag a card between columns to change its status — the CSV is rewritten atomically. Same-column drops are no-ops; dropping into the synthetic Backlog bucket writes an empty status cell. **Thread writes**: the detail panel has a note composer; each save appends one JSONL entry to `<root>/threads/<rowKey>.jsonl` via `local_append_text_file` (O_APPEND, atomic — see §D-L2). **CRM linkage**: a `client_id` (or case-insensitive `client`) column renders an Open client button that launches the CRM app at the contact whose row key matches the cell value. Recent Job Trackers surface on the dashboard. Config under `dropbox-interface:job-tracker:v1`; recents under `dropbox-interface:job-tracker:recents:v1`. **Deferred to v4**: declared status order, undo toast for drag, per-user note author, email-link composer.
- **CRM** (data app, local-only): Pick a folder containing a `contacts.csv`; the table renders sortable + filterable. Each row optionally has an attachments folder at `<root>/files/<rowKey>/*` shown in a detail sidebar (image attachments preview inline via the asset protocol). **Writes**: add row (toolbar `Add row`), edit row (detail panel `Edit`), delete row (detail panel `Delete`, behind a confirm modal — see [`THREAT_MODEL.md`](THREAT_MODEL.md) §D-L3), attach file (detail panel `Attach…`, runs through `local_copy_file`). CSV rewrites are atomic — staged to `.contacts.csv.tmp`, then renamed (§D-L2). Row keys derive from `id` → `name` → first column (case-insensitive); the editor refuses to save a row whose key sanitizes to empty. Recent CRMs surface on the dashboard. Config under `dropbox-interface:crm:v1`; recents under `dropbox-interface:crm:recents:v1`.
...
- [`docs/architecture/upcoming-apps.md`](docs/architecture/upcoming-apps.md) — design sketches for Metadata Guards, Job Tracker, and the Social Media Tester (not built yet — implementation rounds update this file with the final shape).
