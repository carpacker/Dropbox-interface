...
- **Dropbox**: read-only browser for your remote folders. PKCE OAuth flow (no client secret); tokens (access + refresh) are stored in your OS keychain via the `keyring` crate, refreshed automatically when expired, and revoked on disconnect. Scopes requested: `account_info.read`, `files.metadata.read`, `files.content.read`. Backed by `src-tauri/src/dropbox/` — see `service.rs` for the testable HTTP layer and `loopback.rs` for the redirect catcher.
